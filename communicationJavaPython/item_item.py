...
def get_user_history(uid, source):
    ...
    query = (
                "SELECT movie.title, rating.rating FROM movie, rating WHERE rating.user_id = '" + uid + "' AND rating.source = '" + source + "' AND rating.movie_id = movie.movie_id")
    cursor.execute(query)

    for title, rating in cursor:
        print("titile is " +title)
        if title in title_list:
            print("titile is is  " + title)
...
def relevance_hidden_history(user_id, source):
    ...
    if len(history) >= 2:
        ...
        recommendation, votes = get_recommendations_history(visible_history, item_item_matrix,
                                                                                    title_list)
...
#user_id = 'real_user_70'
#source = 'allocine'


if source == 'kaggle':
    title_list = np.load('D:\\PYTHON IDE\\communicationJAVAPYthon\\titles_kaggle.npy').tolist()
    item_item_matrix = np.load('D:\\PYTHON IDE\\communicationJAVAPYthon\\matrix_kaggle.npy')
elif source == 'allocine':
    title_list = np.load('D:\\PYTHON IDE\\communicationJAVAPYthon\\titles_allocine.npy').tolist()
    item_item_matrix = np.load('D:\\PYTHON IDE\\communicationJAVAPYthon\\matrix_allocine.npy')

history = get_user_history(user_id, source)
recommendation, votes = get_recommendations_history(history, item_item_matrix, title_list)
s= ""
