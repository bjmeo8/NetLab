...
def get_user_movies(user_id, th=0):
    ...
    movies = set()
    query = (
        "SELECT DISTINCT title,rating FROM rating,movie WHERE rating.user_id = {} AND movie.movie_id = rating.movie_id AND rating.rating >= {}".format(
            tmp + user_id + tmp, th))
...
def get_user_movies_bis(user_id):
    ...
    movies = set()
    query = (
        "SELECT DISTINCT title,rating FROM rating,movie WHERE rating.user_id = {} AND movie.movie_id = rating.movie_id".format(
            tmp + user_id + tmp))
    cursor.execute(query)
...
def make_recommendation(user_id, source, rth, size, field, th=0.5):
    ...
    if source == 'kaggle':
        user_user_file = open('D:\\PYTHON IDE\\communicationJAVAPYthon\\user_user_kaggle.pkl', 'rb')
        similarity_file = open('D:\\PYTHON IDE\\communicationJAVAPYthon\\similarity_kaggle.pkl', 'rb')
    elif source == 'allocine':
        user_user_file = open('D:\\PYTHON IDE\\communicationJAVAPYthon\\user_user_allocine.pkl', 'rb')
        similarity_file = open('D:\\PYTHON IDE\\communicationJAVAPYthon\\similarity_allocine.pkl',
                               'rb')
...
userid = sys.argv[1]
source = sys.argv[2]
top_recommendation = make_recommendation(userid, source, 3, 0, "none", 0.5)
s = ""
if top_recommendation[0] != "N":
    for i in range(len(top_recommendation)):
        s = s + ";" + top_recommendation[i]
else:
    s = "No Recommendation!"
